...
import AddProductListStock from "./AddProductListStock";
import Swal from "sweetalert2";
const AddProductsStock = ({
  sizes,
  colors,
  setInput,
  input,
  ...
  return (
    <div className="flex flex-col justify-start items-center gap-2">
      ...
      <div className="flex items-center gap-2 w-full text-black">
        ...
        {/* </div> */}
          ...
          className="flex justify-center items-center w-1/4 border-2 text-white p-2 rounded-md"
            onClick={() => {
              const filterColor = input.stock.filter(
                (color) => color.color === colorSelected
              );
              ...
              if (filterColor.length) {
                const findsize = filterColor[0]?.sizes?.filter(
                  (size) => size.size === sizeSelected
                );
                
                if (findsize.length) {
                  //sumar a la cantidad que haya en sizes
                  input.stock.map((color) => {
                    if (color.color === colorSelected) {
                      color.sizes?.map((size) => {
                        if (size?.size === sizeSelected) {
                          size.quantity = parseInt(size?.quantity) + parseInt(clean);
                          setInput({
                            ...input,
                          });
                        }
                  ...
                  input.stock.map((color) => {
                    if (color.color === colorSelected) {
                      color.sizes.push(stockInput.sizes[0]);
                      setInput({
                        ...input,
                      });
                    }
      ...
      <div className="w-full">
        <AddProductListStock input={input} setInput={setInput} />
      </div>
    </div>
  );
};
